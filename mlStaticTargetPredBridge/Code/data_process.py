...
class IntensityMeasure:
    ...
    def make_intensity_measure_file(self, is_save=False, save_file=None):
        """
        Calculate all parameters directly, but save selectively, here the sample of parameter calculations is smaller
        :param is_save: True to save, False not to save.
...
def read_quake_wave(start, end, PGA):
    ...
    for i in tqdm(range(start, end), position=0):
        with open("E:/openSeesModelBridge/quake_wave" + str(PGA) + "/" + str(i) + ".txt", "r") as fp:
...
def read_param(file_name, param_index, PGA, start=0, end=500):
    """
    Read param (one param) into ndarray of size (batch_size).
    :param file_name:
    :param param_index:
    :param start:
    :param end:
    ...
    for i in tqdm(range(start, end), position=0):
        with open("E:/openSeesModelBridge/result" + str(PGA) + "/" + file_name + "-" + str(i) + ".txt", "r") as fp:
            param = np.array(fp.readlines())
            param = [list(map(float, j.split(" "))) for j in param]
            param = [j[param_index] for j in param]
            param = np.abs(np.array(param)).max()  # Get the max num.
            # For defobearing, param_index is 1,3,5,7,9,11.
        data[i - start] = param
    savemat(work_dir + file_name + "-" + str(param_index) + "-" + str(PGA) + ".mat", {
        "data": data  # Size of (1000)
    })


def get_data_set_ml(file_name, param_index, start, end, PGA, features_list=None, is_read_quake=False):
    """
    The workflow of building the data set for machine learning.
    All the IMs and DMs are logged and then scaled into [0.01-1].
    :param file_name: File name of param. Associate with the txt files.
    :param param_index: Index of interested param, e.g. defobearing : 1,3,5,7,9,11
    :param start:
    :param end:
    :return: None. But save as .mat file.
    """
    if is_read_quake:
    ...
    read_param(file_name, param_index, PGA, start, end)
    ...
    y_data = loadmat(work_dir + file_name + "-" + str(param_index) + "-" + str(PGA) + ".mat")["data"]
    ...
    savemat(work_dir + "mlDataset" + file_name + "-" + str(param_index) + "-" + str(PGA) + ".mat", {
        "x": x_data,
        "y": y_data
    })
    print("Done.")
...
if __name__ == "__main__":
    features_list = ['PGD', 'RMSA', 'I_A',
                     'VSI', 'SMA', 'SMV', 'Iv',
                     'Sv(T1)', 'T90', 'FFT']
    # get_data_set_ml("defobearing", 1, 0, 500, 0.7, is_read_quake=True, features_list=features_list)
    # get_data_set_ml("defobearing", 1, 0, 500, 4.0, is_read_quake=True, features_list=features_list)
    # get_data_set_ml("defobearing", 3, 0, 500, 0.7, is_read_quake=False, features_list=features_list)
    # get_data_set_ml("defobearing", 3, 0, 500, 4.0, is_read_quake=False, features_list=features_list)
    # get_data_set_ml("defobearing", 5, 0, 500, 0.7, is_read_quake=False, features_list=features_list)
    # get_data_set_ml("defobearing", 5, 0, 500, 4.0, is_read_quake=False, features_list=features_list)
    # get_data_set_ml("defobearing", 7, 0, 500, 0.7, is_read_quake=False, features_list=features_list)
    # get_data_set_ml("defobearing", 7, 0, 500, 4.0, is_read_quake=False, features_list=features_list)
    # get_data_set_ml("defobearing", 9, 0, 500, 0.7, is_read_quake=False, features_list=features_list)
    # get_data_set_ml("defobearing", 9, 0, 500, 4.0, is_read_quake=False, features_list=features_list)
    # get_data_set_ml("defobearing", 11, 0, 500, 0.7, is_read_quake=False, features_list=features_list)
    # get_data_set_ml("defobearing", 11, 0, 500, 4.0, is_read_quake=False, features_list=features_list)
    get_data_set_ml("hpilebotty#1", 1, 0, 500, 0.7, is_read_quake=False, features_list=features_list)
    get_data_set_ml("hpilebotty#2", 1, 0, 500, 0.7, is_read_quake=False, features_list=features_list)
    get_data_set_ml("hpilebotty#3", 1, 0, 500, 0.7, is_read_quake=False, features_list=features_list)
    get_data_set_ml("hpilebotty#4", 1, 0, 500, 0.7, is_read_quake=False, features_list=features_list)
    get_data_set_ml("hpilebotty#1", 1, 0, 500, 4.0, is_read_quake=False, features_list=features_list)
    get_data_set_ml("hpilebotty#2", 1, 0, 500, 4.0, is_read_quake=False, features_list=features_list)
    get_data_set_ml("hpilebotty#3", 1, 0, 500, 4.0, is_read_quake=False, features_list=features_list)
    get_data_set_ml("hpilebotty#4", 1, 0, 500, 4.0, is_read_quake=False, features_list=features_list)
